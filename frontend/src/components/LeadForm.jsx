...
const LeadForm = ({ variant = "inline", defaultService = "", title = "Talk to a care coordinator" }) => {
  const [form, setForm] = useState({ name: "", phone: "", email: "", city: "", service: defaultService, message: "", urgency: "standard" });
  ...
  return (
    <form onSubmit={submit} data-testid={`lead-form-${variant}`} className="space-y-3.5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <Field data-testid={`${variant}-lead-name`} value={form.name} onChange={(v) => setField("name", v)} placeholder="Full name" />
        <Field data-testid={`${variant}-lead-phone`} value={form.phone} onChange={(v) => setField("phone", v)} placeholder="Phone" type="tel" />
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <Field data-testid={`${variant}-lead-email`} value={form.email} onChange={(v) => setField("email", v)} placeholder="Email (optional)" type="email" />
        <Select data-testid={`${variant}-lead-city`} value={form.city} onChange={(v) => setField("city", v)} placeholder="City">
          {LOCATIONS.map((l) => (<option key={l.slug} value={l.name}>{l.name}</option>))}
        </Select>
      </div>
      <Select data-testid={`${variant}-lead-service`} value={form.service} onChange={(v) => setField("service", v)} placeholder="Service required">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {["standard","urgent","emergency"].map((u) => (
          <button key={u} type="button" onClick={() => setField("urgency", u)}
            className={`px-3.5 py-1.5 rounded-full text-xs font-medium border transition-colors ${form.urgency === u ? "bg-primary text-primary-foreground border-primary" : "border-border text-muted-foreground hover:bg-muted"}`}
            data-testid={`${variant}-lead-urgency-${u}`}
          >{u === "standard" ? "Standard · today" : u === "urgent" ? "Urgent · < 4h" : "Emergency · now"}</button>
        ))}
      </div>

      <button data-testid={`${variant}-lead-submit`} disabled={loading} type="submit" className="btn-gold w-full mt-1">
        {loading ? "Sending…" : (<>Request callback <ArrowRight size={16}/></>)}
      </button>
      <p className="text-[11px] text-muted-foreground text-center">By submitting, you consent to be contacted by our care team.</p>
    </form>
  );
};
